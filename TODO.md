...
- [ ] Add optional `user_id: Optional[int] = None` query parameter to `list_private_ai_keys_by_region` in `app/api/private_ai_keys.py`
...
- [ ] Implement `user_id` filter logic for admin users
...
- [ ] Update the endpoint docstring to document the new `user_id` query parameter and its admin-only behaviour
