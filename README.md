...
sets of notes describe the release, and they are produced in different ways for
different readers.

**The deploy pull request body** is generated by
`.github/scripts/deploy-pr-body.sh`, straight from `git log` and
`git diff --name-status` over `main..dev`. It lists the schema migrations by
name and flags every changed file that decides how production runs —
`.lagoon.yml`, `docker-compose.yml`, `helm/`, the Dockerfiles,
`app/core/config.py`. No model is involved, so it cannot be truncated and cannot
describe a deploy as carrying no migrations when it carries three. Because it is
cheap, it is rewritten on every push to `dev` while the deploy PR is open, and
always describes what merging it would deploy.

**The GitHub Release page** gets user-facing notes written by a model reached
through the LiteLLM proxy, from the diff of the released tag range. Prose earns
its place here: the reader is a user of the API, not a deploy approver. The
prompt lives in `.github/prompts/changelog-user.md`, and the step needs the
`LITELLM_API_KEY` repository secret. Without it, this part is skipped and
nothing else changes. If the release holds nothing a user can see, the model
says so and the notes are left off.
...
- **The deploy PR is `dev` → `main`, not tag → `main`.** Commits that land on
  `dev` after the tag ship with it, so the deploy can hold more than the tag
  names. The body is rewritten on every push to `dev`, so read it rather than the
  title to see what actually ships.
