...
say "Assert /spend history includes periodic_transactions with both payment types"
api GET "/spend/${REGION_ID}/team/${TEAM_ID}/history"
[[ "$HTTP_STATUS" == "200" ]] || fail "history endpoint failed: $HTTP_STATUS $HTTP_BODY"
TX_COUNT="$(echo "$HTTP_BODY" | jq -r '.periodic_transactions | length')"
[[ "$TX_COUNT" -ge 2 ]] || fail "expected >=2 periodic_transactions, got ${TX_COUNT}"
HAS_SUB="$(echo "$HTTP_BODY" | jq -r '[.periodic_transactions[] | select(.payment_type=="subscription")] | length')"
HAS_TOPUP="$(echo "$HTTP_BODY" | jq -r '[.periodic_transactions[] | select(.payment_type=="topup")] | length')"
[[ "$HAS_SUB" -ge 1 ]] || fail "history missing subscription transaction"
[[ "$HAS_TOPUP" -ge 1 ]] || fail "history missing topup transaction"
pass "history periodic_transactions contains subscription + topup"
