...
say "Plan: PERIODIC top-up API E2E"
say "Tests:"
say "  1) Create periodic team and key in target region"
say "  2) Create periodic top-up purchase (201)"
say "  3) Verify duplicate stripe_payment_id rejection (409)"
say "  4) Verify periodic status topup_remaining_cents > 0"
...
step "Ensure team-region association exists"
api POST "/regions/${REGION_ID}/teams/${TEAM_ID}" || true

step "Create one team key in target region"
...
record_ok "Team key created in region ${REGION_ID} (key_id=${KEY_ID})"

step "Test #1: successful periodic top-up purchase"
...
value "stripe_payment_id" "$TOPUP_ID"
record_ok "Periodic top-up accepted (201)"

step "Test #2: duplicate stripe_payment_id is rejected"
check "Second POST with same stripe_payment_id returns 409"
api POST "/budgets/region/${REGION_ID}/teams/${TEAM_ID}/purchase/periodic" "$(jq -nc --arg sid "$TOPUP_ID" '{amount_cents:500,currency:"USD",purchased_at:(now|todateiso8601),stripe_payment_id:$sid}')"
[[ "$HTTP_STATUS" == "409" ]] || fail "expected 409 duplicate, got $HTTP_STATUS $HTTP_BODY"
pass "Duplicate top-up rejected with 409"
record_ok "Duplicate stripe_payment_id rejected (409)"

step "Test #3: periodic status shows non-zero top-up remaining"
...
record_ok "Periodic status topup_remaining_cents=${TOPUP_REMAINING}"
