...
def test_get_portal_existing_customer(
    mock_create_portal, client, db, test_team, team_admin_token
):
    """
    ...
    mock_create_portal.assert_called_once_with(
        "cus_123", f"http://localhost:3000/teams/{test_team.id}/dashboard"
    )


@patch("app.api.billing.create_portal_session", new_callable=AsyncMock)
def test_get_portal_with_custom_return_url(
    mock_create_portal, client, db, test_team, team_admin_token
):
    """
    GIVEN: A team with an existing Stripe customer ID
    ...
    assert response.status_code == 303
    assert response.headers["location"] == mock_portal_url
    mock_create_portal.assert_called_once_with("cus_123", custom_return_url)


@patch("app.api.billing.create_portal_session", new_callable=AsyncMock)
@patch("app.api.billing.create_stripe_customer", new_callable=AsyncMock)
def test_get_portal_create_customer(
    mock_create_customer, mock_create_portal, client, db, test_team, team_admin_token
):
    # Arrange
...
@patch("app.api.billing.create_portal_session", new_callable=AsyncMock)
def test_get_portal_as_system_admin(
    mock_create_portal, client, db, test_team, admin_token
):
    # Arrange
    test_team.stripe_customer_id = "cus_123"
    db.add(test_team)
...
def test_get_pricing_table_session_existing_customer(
    mock_get_session, client, db, test_team, team_admin_token
):
    # Arrange
    test_team.stripe_customer_id = "cus_123"
    db.add(test_team)
...
def test_get_pricing_table_session_create_customer(
    mock_create_customer, mock_get_session, client, db, test_team, team_admin_token
):
    # Arrange
    test_team.stripe_customer_id = None
    db.add(test_team)
...
@patch("app.services.stripe.stripe.api_key", "sk_test_mock")
@patch("app.api.billing.get_pricing_table_secret")
def test_get_pricing_table_session_as_system_admin(
    mock_create_session, client, db, test_team, admin_token
):
    # Arrange
    test_team.stripe_customer_id = "cus_123"
    db.add(test_team)
...
@patch("app.services.stripe.stripe.api_key", "sk_test_mock")
@patch("app.api.billing.get_pricing_table_secret")
def test_get_pricing_table_session_stripe_error(
    mock_get_session, client, db, test_team, team_admin_token
):
    # Arrange
    test_team.stripe_customer_id = "cus_123"
...
@patch("app.api.billing.create_stripe_customer", new_callable=AsyncMock)
def test_create_team_subscription_success_existing_customer(
    ...
    mock_create_subscription.assert_called_once_with(
        customer_id="cus_123", product_id=test_product.id
    )
...
def test_create_team_subscription_success_new_customer(
    ...
    mock_create_subscription.assert_called_once_with(
        customer_id=mock_customer_id, product_id=product_id
    )
...
def test_create_team_subscription_product_not_found(client, db, test_team, admin_token):
    ...
    assert (
        ...
        == f"Product with ID {non_existent_product_id} not found in database"
    )


def test_create_team_subscription_unauthorized(
    client, db, test_team, test_product, team_admin_token
):
...
@patch("app.api.billing.create_zero_rated_stripe_subscription", new_callable=AsyncMock)
@patch("app.api.billing.create_stripe_customer", new_callable=AsyncMock)
def test_create_team_subscription_stripe_error(
    mock_create_customer,
    mock_create_subscription,
    client,
    db,
...
def test_delete_subscription_for_team(
    mock_cancel_subscription,
    mock_get_subscribed_products_for_customer,
    client,
    admin_token,
    test_team,
    test_product,
    ...
    product_id = test_product.id
    team_product = DBTeamProduct(team_id=team_id, product_id=product_id)
    db.add(team_product)
    test_team.stripe_customer_id = "cus_123"
    db.add(test_team)
    db.commit()
...
def test_delete_subscription_product_not_found(client, admin_token, test_team):
    ...
    assert response.status_code == 400
    assert (
        "Product with ID nonexistent_product not found in database"
        in response.json()["detail"]
    )


def test_delete_subscription_no_association(
    client, admin_token, test_team, test_product, db
):
    """
    ...
    assert response.status_code == 400
    assert (
        f"Team {test_team.id} is not associated with product {test_product.id}"
        in response.json()["detail"]
    )


def test_delete_subscription_no_stripe_customer(
    client, admin_token, test_team, test_product, db
):
    """
...
@patch("app.api.billing.get_subscribed_products_for_customer", new_callable=AsyncMock)
@patch("app.api.billing.cancel_subscription", new_callable=AsyncMock)
def test_delete_subscription_stripe_get_products_error(
    mock_cancel_subscription,
    mock_get_subscribed_products_for_customer,
    client,
    admin_token,
...
@patch("app.api.billing.get_subscribed_products_for_customer", new_callable=AsyncMock)
@patch("app.api.billing.cancel_subscription", new_callable=AsyncMock)
def test_delete_subscription_stripe_cancel_error(
    mock_cancel_subscription,
    mock_get_subscribed_products_for_customer,
    client,
    admin_token,
    ...
    mock_get_subscribed_products_for_customer.return_value = [
        ("sub_1234", test_product.id)
    ]
