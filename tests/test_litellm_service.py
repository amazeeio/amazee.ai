...
@patch("httpx.AsyncClient")
def test_create_key_omits_budget_duration_when_none(
...
@patch("httpx.AsyncClient")
def test_update_budget_omits_budget_duration_when_none(
    mock_client_class, test_region, mock_httpx_post_client
):
    ...
    asyncio.run(
        service.update_budget(
            "test-token", budget_duration=None, budget_amount=100.0, duration="120d"
        )
    )
    sent_payload = mock_httpx_post_client.post.call_args.kwargs["json"]
