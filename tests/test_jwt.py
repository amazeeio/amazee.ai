...
def test_validate_jwt_valid_token_auth_header(client, test_team_admin):
    ...
    response = client.get(
        "/auth/validate-jwt", headers={"Authorization": f"Bearer {token}"}
    )
    assert response.status_code == 200
    data = response.json()
    assert "access_token" in data
...
def test_validate_jwt_invalid_auth_header_format(client):
    ...
    response = client.get(
        "/auth/validate-jwt", headers={"Authorization": "invalid-token"}
    )
    assert response.status_code == 401
    data = response.json()
    assert "detail" in data
...
def test_get_team_admin_email_no_admin_found(db, test_team):
    ...
    db.query(DBUser).filter(
        DBUser.team_id == test_team.id, DBUser.role == "admin"
    ).delete()
    db.commit()
    ...
    with pytest.raises(
        ValueError,
        match=f"No admin user found for team {test_team.name} \\(ID: {test_team.id}\\)",
    ):
        get_team_admin_email(db, test_team)


def test_get_team_admin_email_multiple_admins_returns_first(
    db, test_team, test_team_admin
):
...
def test_get_team_admin_email_different_team_ignored(db, test_team, test_team_admin):
    ...
    other_team = DBTeam(name="Other Team", admin_email="other_admin@test.com")
    db.add(other_team)
    db.commit()
...
def test_get_team_admin_email_team_without_users(db):
    ...
    empty_team = DBTeam(name="Empty Team", admin_email="empty_admin@test.com")
    db.add(empty_team)
    db.commit()
    ...
    with pytest.raises(
        ValueError,
        match=f"No admin user found for team {empty_team.name} \\(ID: {empty_team.id}\\)",
    ):
