...
def test_view_spend_with_missing_fields(
    ...
    mock_response.json.return_value = {
        "info": {
            # Missing spend field - should default to 0.0
            "expires": "2024-12-31T23:59:59Z",
            "created_at": "2024-01-01T00:00:00Z",
            "updated_at": "2024-01-02T00:00:00Z",
            # Missing max_budget and budget_duration - should default to None
        }
    }
...
@patch("httpx.AsyncClient")
def test_create_llm_token_pool_mode_expired_returns_402(
    mock_client_class,
    client,
    team_admin_token,
    test_region,
...
@patch("httpx.AsyncClient")
def test_create_llm_token_pool_mode_exhausted_returns_402(
    mock_client_class,
    client,
    team_admin_token,
    test_region,
...
@patch("httpx.AsyncClient")
def test_create_llm_token_pool_mode_omits_budget_duration(
    mock_client_class,
    client,
    team_admin_token,
    test_region,
...
def test_get_private_ai_key_success(
    ...
    region_id = test_region.id
    region_name = test_region.name
    # Refresh the team to ensure it's bound to the current session
...
def test_create_private_ai_key_cleanup_failure_handling(
    ...
    # Use the httpx POST client fixture but customize its behavior
    mock_client_class.return_value = mock_httpx_post_client
    # Mock cleanup failure - the second call will be for cleanup
    # First call succeeds, second call fails
    postsuccess = Mock()
    postsuccess.status_code = 200
    postsuccess.json.return_value = {"key": "test-private-key-123"}
