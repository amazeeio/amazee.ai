...
def test_update_team_budget_mode_requires_system_admin(
    ...
    assert response.status_code == 403
    assert (
        response.json()["detail"] == "Only system administrators can change budget mode"
    )
...
def test_update_team_switch_to_pool_resets_budget_and_reconciles_keys(
    mock_propagate, client, admin_token, db, test_team, test_region
):
...
def test_update_team_switch_to_pool_restores_region_counters_from_ledger(
    ...
    restored_primary = (
        ...
        .filter(
            DBTeamRegion.team_id == team.id, DBTeamRegion.region_id == test_region.id
        )
        .first()
    )
    ...
    restored_extra = (
        db.query(DBTeamRegion)
        .filter(
            DBTeamRegion.team_id == team.id, DBTeamRegion.region_id == extra_region.id
        )
        .first()
    )
