...
revision: str = "e5d0ea6833ff"
down_revision: Union[str, None] = "95ac6f88662f"
...
def downgrade() -> None:
    ...
    if insp.has_table("pool_purchases"):
        ...
        if "ix_pool_purchases_stripe_payment_id" in indexes:
            op.drop_index(
                "ix_pool_purchases_stripe_payment_id",
                table_name="pool_purchases",
            )
        if "ix_pool_purchases_region_id" in indexes:
            op.drop_index(
                "ix_pool_purchases_region_id",
                table_name="pool_purchases",
            )
