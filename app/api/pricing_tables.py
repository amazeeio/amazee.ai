...
router = APIRouter(tags=["pricing-tables"])
...
async def create_pricing_table(
    pricing_table: PricingTableCreate, db: Session = Depends(get_db)
):
    """
    ...
    stripe_publishable_key = (
        pricing_table.stripe_publishable_key or settings.STRIPE_PUBLISHABLE_KEY
    )
...
async def get_pricing_table(
    ...
    pricing_table = (
        db.query(DBPricingTable)
        .filter(
            DBPricingTable.table_type == table_type, DBPricingTable.is_active.is_(True)
        )
        .first()
    )
...
@router.delete("/", dependencies=[Depends(get_role_min_system_admin)])
async def delete_pricing_table(table_type: str, db: Session = Depends(get_db)):
    """
...
@router.get(
    "/list",
    response_model=PricingTablesResponse,
    dependencies=[Depends(get_role_min_system_admin)],
)
async def get_all_pricing_tables(db: Session = Depends(get_db)):
    """
    ...
    pricing_tables = (
        db.query(DBPricingTable).filter(DBPricingTable.is_active.is_(True)).all()
    )
